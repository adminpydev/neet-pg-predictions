...
NORES = {"Deemed (Self-financed merit)", "NRI"}
def agg(d):
    op = d["rank"] if d.name[5] in NORES else d[d.cat.isin(["Open", "OBC"]) | d.cand.isin(["General", "OBC"])]["rank"]
    return pd.Series({"Allotments": len(d), "Opening Rank (all cat)": d["rank"].min(), "Closing Rank (all cat)": d["rank"].max(),
                      "OBC-eligible Closing Rank": op.max() if len(op) else None, "Rounds": ", ".join(sorted(d["round"].unique()))})
T = {}
for src in ["2024", "2025 Stray"]:
    T[src] = A[A.src == src].groupby(g).apply(agg, include_groups=False).reset_index()
C = T["2024"].merge(T["2025 Stray"][g + ["Closing Rank (all cat)", "OBC-eligible Closing Rank"]].rename(columns={
    "Closing Rank (all cat)": "2025 Stray Closing (all cat)", "OBC-eligible Closing Rank": "2025 Stray OBC-eligible Closing"}), on=g, how="outer")
C["Allotments"] = C["Allotments"].fillna(0).astype(int)
ELIG = {"All India 50%", "Deemed (Self-financed merit)", "DNB"}
def chance(row):
    op = pd.Series([row["OBC-eligible Closing Rank"], row["2025 Stray OBC-eligible Closing"]]).max()
    ...
    if pd.isna(op): return pd.Series(["Unclear (no OBC/General data)", None, f"All-category closing {allc:.0f}; OBC cutoff may be lower" if pd.notna(allc) else ""])
    b, basis = op, "Last rank allotted to General/OBC candidate or Open/OBC seat (Deemed: any)"
    lab = "High" if b >= RANK * 1.15 else "Good" if b >= RANK else "Borderline" if b >= RANK * 0.9 else "Low"
    return pd.Series([lab, b, basis])
C[["Chance (OBC)", "Rank used for chance", "Basis"]] = C.apply(chance, axis=1)
order = {"High": 0, "Good": 1, "Borderline": 2}
C = C.sort_values(["Stream", "Degree", "Rank used for chance"], ascending=[True, True, True])
P = C[C["Chance (OBC)"].isin(["High", "Good", "Borderline"])].copy()
P["o"] = P["Chance (OBC)"].map(order); P = P.sort_values(["Stream", "o", "Rank used for chance"]).drop(columns="o")

SS = P.pivot_table(index="Stream", columns="Sector", values="Institute", aggfunc="count", fill_value=0)
SS["Total options"] = SS.sum(axis=1)
best = P[P["Chance (OBC)"].isin(["High", "Good", "Borderline"])].sort_values("Rank used for chance").groupby("Stream").head(3)
...
C.to_pickle("mcc_obc.pkl"); S25.to_pickle("s25.pkl"); raise SystemExit
...
print(len(A), len(C), len(P)); print(C["Chance (OBC)"].value_counts().to_string())
